...
fn main() {
    let input = fs::read_to_string("/Users/dmsnell/code/aoc/aoc2024/priv/day4.txt")
        .unwrap()
        .as_bytes()
        .to_vec();
    for _ in 1..10000 {
        crate::days::day4::p1(&input);
        ()
    }
    let now = Instant::now();
    let sum = crate::days::day4::p1(&input);
    let duration = now.elapsed();

    println!(
        "\x1b[90mFound \x1b[34m{:?}\x1b[90m in \x1b[3;33m{:?}\x1b[0;90m.\x1b[m",
        sum, duration
    );
}

fn day3p2(input: &str) -> usize {
    let mut state: State = State {
        data: input,
        state: MachineState::On,
        next: NextToken::Instruction,
        sum: 0,
    };

    while MachineState::On == next(&mut state) {}

    state.sum
}

fn next(state: &mut State) -> MachineState {
    match state.next {
        NextToken::Do => next_do(state),
        NextToken::Instruction => next_instruction(state),
        NextToken::Mul1 => mul_int1(state),
        NextToken::Mul2(int1) => mul_int2(state, int1),
    }
}

fn next_instruction(state: &mut State) -> MachineState {
    debug_assert_eq!(state.state, MachineState::On);

    if state.data.starts_with("don't()") {
        let (_dont, rest) = state.data.split_at(7);
        debug_assert_eq!(_dont, "don't()");
        state.data = rest;
        state.state = MachineState::Off;
        state.next = NextToken::Do;
        return MachineState::On;
    } else if state.data.starts_with("mul(") {
        let (_mul, rest) = state.data.split_at(4);
        debug_assert_eq!(_mul, "mul(");
        state.data = rest;
        state.next = NextToken::Mul1;
        return MachineState::On;
    }

    return match state.data.split_at_checked(1) {
        Some((_c, rest)) => {
            state.data = rest;

            let (prefix, middle, suffix) = rest.as_bytes().as_simd::<LANES>();
            for i in 0..(prefix.len()) {
                match prefix.get(i).unwrap() {
                    b'd' | b'm' => {
                        state.data = rest.split_at(i).1;
                        return MachineState::On;
                    }
                    _ => (),
                }
            ...
            // 'm' = 0x6D = 0110 1101
            //              0110 x10x
            let mask = Simd::<u8, LANES>::splat(0x64);
            let wipe = Simd::<u8, LANES>::splat(0xF6);
            ...
            let positions = Simd::<u8, LANES>::load_or_default(&indices);
            for (i, &sample) in middle.iter().enumerate() {
                let candidates = (sample & wipe).simd_eq(mask);
                let at = candidates.select(positions, highs).reduce_min();
                if at < 0xFF {
                    let offset = prefix.len() + i * LANES + (at as usize);
                    state.data = rest.split_at(offset).1;
                    return MachineState::On;
                }
            }

            for i in 0..(suffix.len()) {
                match suffix.get(i).unwrap() {
                    b'd' | b'm' => state.data = rest.split_at(rest.len() - suffix.len() + i).1,
                    _ => (),
                }
            }
        ...
        None => MachineState::Off,
    };
}

fn next_do(state: &mut State) -> MachineState {
    debug_assert_eq!(state.state, MachineState::Off);

    return match state.data.find("do()") {
        Some(at) => {
            let (_do, rest) = state.data.split_at(at + 4);
            state.data = rest;
            state.state = MachineState::On;
            state.next = NextToken::Instruction;
            MachineState::On
        }
        None => MachineState::Off,
    };
...
fn mul_int1(state: &mut State) -> MachineState {
    debug_assert_eq!(state.state, MachineState::On);

    // 0 -> 0x30 = 0011 0000
    // 9 -> 0x39 = 0011 1001
    //             0011 xxxx (symbols and numbers)
    // Run in u32x4, which should allow to:
    //  - subtract b'0' from each digit
    //  - multiply by [1000, 100, 10, 1]
    //  - sum the group
    //
    // Except…the number groups tend to be three digits each, so there’s
    // not much incentive to vectorize them with wide lanes, other than
    // perhaps to multiply them together.

    let mut int1: usize = 0;
    for (i, c) in state.data.bytes().enumerate() {
        match c {
            b'0'..=b'9' => int1 = int1 * 10 + ((c - b'0') as usize),
            b',' if i > 0 => {
                let (_comma, rest) = state.data.split_at(i + 1);
                debug_assert_eq!(&_comma[(_comma.len() - 1)..], ",");
                state.data = rest;
                state.next = NextToken::Mul2(int1);
                return MachineState::On;
            }
            _ => {
                let (_corrupt, rest) = state.data.split_at(i + 1);
                state.data = rest;
                state.next = NextToken::Instruction;
                return MachineState::On;
            }
...
fn mul_int2(state: &mut State, int1: usize) -> MachineState {
    debug_assert_eq!(state.state, MachineState::On);

    let mut int2: usize = 0;
    for (i, c) in state.data.bytes().enumerate() {
        match c {
            b'0'..=b'9' => {
                int2 = int2 * 10 + ((c - b'0') as usize);
            }
            b')' if i > 0 => {
                let (_closing, rest) = state.data.split_at(i + 1);
                debug_assert_eq!(&_closing[(_closing.len() - 1)..], ")");
                state.data = rest;
                state.sum += int1 * int2;
                state.next = NextToken::Instruction;
                return MachineState::On;
            ...
            _ => {
                let (_c, rest) = state.data.split_at(1);
                state.data = rest;
                state.next = NextToken::Instruction;
                return MachineState::On;
            }
...
#[test]
fn part2_test_case() {
    assert_matches!(day3p2(r"mul(810,622)mul(163,901)"), 650683)
}

#[test]
fn part2_example_test() {
    assert_matches!(
        day3p2(r"xmul(2,4)&mul[3,7]!^don't()_mul(5,5)+mul(32,64](mul(11,8)undo()?mul(8,5))\n"),
        48
    );
}
